<div id="product-list-container">
    <div class="listing-topbar">
        <p style="font-size: var(--font-size-sm); color: var(--color-text-muted);">
            ...
            @if(isset($queryText) && $queryText) cho từ khóa "<strong>{{ $queryText }}</strong>"@endif
        </p>
        <select class="form-control" style="width: auto;" id="sort-select-ajax">
    ...
    <div class="pagination ajax-pagination">
        @if (!$sachs->onFirstPage())
            <a href="#" data-page="{{ $sachs->currentPage() - 1 }}"><span class="material-icons">chevron_left</span></a>
        @endif
        ...
        @if ($sachs->hasMorePages())
            <a href="#" data-page="{{ $sachs->currentPage() + 1 }}"><span class="material-icons">chevron_right</span></a>
        @endif
    </div>
</div>
