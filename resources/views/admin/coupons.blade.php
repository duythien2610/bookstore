            ...
            <div class="card" style="padding: var(--space-6);">
                ...
                <form action="{{ route('admin.coupons.store') }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <div style="font-size: var(--font-size-xs); color: var(--color-text-muted); margin-top: 4px;">(Bỏ trống nếu mã không có hạn)</div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Số Lượng Giới Hạn</label>
                        <input type="number" name="so_luong" class="input" min="1">
                        <div style="font-size: var(--font-size-xs); color: var(--color-text-muted); margin-top: 4px;">(Bỏ trống nếu không giới hạn số lượng user dùng)</div>
                    </div>
                            ...
                            <td>
                                <div style="display:flex; gap: 4px;">
                                    <form action="{{ route('admin.coupons.toggle', $c->id) }}" method="POST">
                                        ...
                                        <button class="btn btn-sm btn-ghost" title="{{ $c->trang_thai ? 'Tắt' : 'Bật' }} mã này">
                                            <span class="material-icons {{ $toggleColor }}">{{ $c->trang_thai ? 'pause' : 'play_arrow' }}</span>
                                        </button>
                                    </form>
                                    <form action="{{ route('admin.coupons.destroy', $c->id) }}" method="POST" onsubmit="return confirm('Bạn chắc chắn muốn xóa mã {{ $c->ma_code }}? Thao tác không thể hoàn tác!')">
                                        @csrf @method('DELETE')
                                        <button class="btn btn-sm btn-ghost" title="Xóa vĩnh viễn"><span class="material-icons" style="color:var(--color-danger);">delete</span></button>
                                    </form>
                                </div>
                            </td>
                        </tr>
